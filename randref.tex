...
In order to apply thge complete planning algorithm described above we must provide definitions for each of the subroutines mentioned in Algorithm 1. There are many ways to implement these functions. For example, SFCRA-14 uses a backtracking search over a discrete set of instantiations to implement \textsc{RefineNode}. Since we want to apply RL to learn policies for refinement,
...
which implements \textsc{NDGetInstantiation} in Alg.\,\ref{alg:complete}. The main difference between our implementation and the pseudo-code in Alg.\,\ref{alg:complete} is that we use information about previous failed motion planning attempts to guide selecting a next instantiation.
...
\begin{small}
  ...
  \nl \textsc{Resample}($failPred.params$) } }}q
...
resamples its value. Between the two of them, \textsc{InitRefinement} and \textsc{Resample} define \textsc{NDGetInstantiation} for our implementation. \textsc{GetError} iterates through the steps of the plan, checks precondition and trajectory feasibility and returns a failed action index and associated predicate. 
