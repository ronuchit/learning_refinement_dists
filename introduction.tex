...
high-level (logic-based) plans and their low-level (motion-planning)
refinements. We show how statistical learning techniques can be used
to make this process more efficient than previous hand-coded
approaches for identifying the space of relevant values for continuous
action arguments such as grasping poses.
...
To learn how to motion plan low-level actions, we train distributions
that propose values for continuous parameters that are likely to lead
to successful trajectories. We train these distributions through an
application of reinforcement learning (RL) to the meta-level problem
of refining a fixed high-level plan. Our approach draws inspiration
...
new values for parameters. We implement our approach using methods
