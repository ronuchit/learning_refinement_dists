...
of every movable object. A task and motion planning problem adds to
this formulation more abstract concepts including \emph{fluents}
(logical properties that may vary across configurations) and
\emph{actions} (operations that the agent may choose to execute,
resulting in changes in the configuration and the set of fluents that
are true. Each action may require motion planning prior to execution.
The overall problem is to compute the sequence of actions
...
e.g., \emph{grasp(can$_1$, left, g$_1$, m$_1$)}, where the variables
...
\emph{literals}), e.g. \emph{empty(gripper$_l$)}.
...
collision-free. Other trajectory constraints, such as dynamic
feasibility may also be included as action preconditions by expanding
the set of fluents in the problem definition.

A solution to a task planning problem is a sequence of instantiated
actions $a_{0}, a_{1}, ..., a_{n} \in \U$ such that every action is
feasible when it is applied on states starting with $\I$, and the
