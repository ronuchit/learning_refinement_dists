...
A motion planning problem is defined by a configuration space $C$ for a robot
and all movable objects in its environment, along with initial and final configurations
$c_{0}, c_{T} \in C$. The solution to a motion planning problem is a collision-free trajectory for
the robot that connects $c_{0}$ and $c_{T}$.
...
\begin{defn}
Formally, we define an {\sc mdp} as a tuple $\langle \St, \A, T, R, \gamma, \Prob \rangle$, where
\begin{tightlist}
...
\end{tightlist}
\end{defn}
A solution to an {\sc mdp} is a policy, $\pi: \St \rightarrow \A$, that maps states to
...
affect the state. There is a large body of research on {\sc rl}, and
standard techniques include value function approximation, which uses methods such as temporal difference
learning, and direct policy estimation, which encompasses both gradient-based
