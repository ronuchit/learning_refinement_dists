...
%[[SS: using standard notation, replace symbolic plan throughout with
%high-level plan]]
...
\begin{small}
    ...
    \nl \For{trial in 1 ...}{
      \nl  \For{j in 1 .. trial}{
        \tcc{\footnotesize Traverse graph of plans, initially with just one plan:
          $\epsilon$.}
        \nl $u \leftarrow$ \textsc{GetNextNode}(PRGraph)\;
        \nl mode $\leftarrow$ NDChoice\{refine, add\}\;
  ...
  \myproc{\textsc{RefineNode}($u$, $j$)}{
    ...
    \tcc{\footnotesize resourceLimit($j$) is a  monotonically increasing function of $j$}
    \nl  $MP \leftarrow$ GetMotionPlan($\sigma$, $\pi$, resourceLimit($j$))\;
    \nl \If{MP $\ne$ NULL}{
      \nl return success\;
    }
    }\;
  ...
  \myproc{\textsc{AddChild}($u$, $j$)}{
    ...
    \nl mode $\leftarrow$ NDChoice\{random, error\}\;
    \nl \eIf{mode == error} {
    ...
    \nl $\pi'\leftarrow$ getClassicalPlan(newState)\;
    \nl AddNodeToPRGraph($\sigma$, stepNum, $\pi'$)\;
  }
\end{small}
\caption{Algorithm for task and motion planning}
\label{alg:complete}
\end{algorithm}
