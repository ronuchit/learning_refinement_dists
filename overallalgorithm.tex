...
\begin{figure}[t]
  ...
  \node (1) at (-1, 0) {1. Move \{\emph{pose}$_{init}$, \emph{pose$_{A}$}\}\\2. Grasp A \{\emph{pose$_{A}$}\}};
  \node (2) at (-3, -3) {1. Move \{\emph{pose}$_{init}$, \emph{pose$_{B}$}\}\\2. Grasp B \{\emph{pose$_{B}$}\}\\
    3. Move \{\emph{pose}$_{B}$, \emph{pose$_{A}$}\}\\4. Grasp A \{\emph{pose$_{A}$}\}};
  \node (3) at (1.5, -3) {1. Move \{\emph{pose}$_{init}$, \emph{pose$_{C}$}\}\\2. Grasp C \{\emph{pose$_{C}$}\}\\
    3. Move \{\emph{pose}$_{C}$, \emph{pose$_{A}$}\}\\4. Grasp A \{\emph{pose$_{A}$}\}};
  \draw [postaction={decorate}] (-2, -0.5) -- (2) node [midway, left, draw=none, text=red] {B obstructs\\path to A};
  \draw [postaction={decorate}] (0, -0.5) -- (3) node [midway, right, draw=none, text=red] {C obstructs\\path to A};
  \end{tikzpicture}
...
We introduce a complete algorithm that maintains a \emph{plan
  refinement graph} ({\sc prg}). \figref{fig:prg} illustrates a simple example with 3 high-level plans.
...
{\sc prg}. We will show that using trained guided search heuristics with the
{\sc prg} can lead to performance improvements.
